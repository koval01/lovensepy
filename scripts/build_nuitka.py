...
def main() -> None:
    ...
    if is_macos:
        ...
        # TCC aborts the process if CoreBluetooth is used without this plist key (Bleak).
        _ble_usage = (
            "LovensePy Service uses Bluetooth Low Energy to discover and connect to "
            "Lovense devices when BLE or hybrid mode is enabled."
        )
        macos_nuitka_opts = [
            "--macos-app-icon=none",
            "--macos-app-protected-resource=NSBluetoothAlwaysUsageDescription:"
            + _ble_usage,
        ]
    else:
        out_dir = "dist-nuitka"
